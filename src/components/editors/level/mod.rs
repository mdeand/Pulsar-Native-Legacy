...
impl Element for MySurface {
    ...
    fn request_layout(
        ...
        (cx.request_measured_layout(Default::default(), |_, _, _| gpui::Size { width: gpui::Pixels::from(100.), height: gpui::Pixels::from(100.) }), ())
    }
...
impl EditorView for LevelEditorView {
  fn render(&self, _cx: &mut ViewContext<TabBar>) -> AnyElement {
    ...
    let elapsed_time = ANIMATION_START_TIME.elapsed().as_secs_f32();
    let fps = if frame_time > 0 { 1000.0 / frame_time as f32 } else { 0.0 };
